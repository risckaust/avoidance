...
double StarPlanner::treeCostFunction(int node_number) {
  ...
  int goal_z = azimuthAnglefromCartesian(toPoint(goal_), origin_position);
  int goal_e = elevationAnglefromCartesian(toPoint(goal_), origin_position);
...
double StarPlanner::treeHeuristicFunction(int node_number) {
  geometry_msgs::Point node_position = tree_[node_number].getPosition();
  int goal_z = azimuthAnglefromCartesian(toPoint(goal_), node_position);
  int goal_e = elevationAnglefromCartesian(toPoint(goal_), node_position);
...
void StarPlanner::buildLookAheadTree() {
  ...
  while (n < n_expanded_nodes_) {
    ...
    Eigen::Vector3f temp_sphere_center;  // unused
